...
### W40 — Cross-layer tracker lag: an item shows OPEN in Linear/Notion/backlog while it is already SHIPPED in the repo (no mechanical slug↔tracker join) (2026-06-29)

**Surfaced by:** the 2026-06-29 cross-layer reconciliation. A request to "start R8" (a dev-env backlog item) led — only because of a verify-first read of the target — to discovering R8 (`rotate-feature-logs.py`) was **already shipped** (PR #429). A follow-on agent sweep then found the same lag across **~25 more Linear items** still in Backlog/Open despite being complete in the repo: e.g. FIT-94 (F14), FIT-95 (F1), FIT-102 (F18), FIT-105 (F3), FIT-151 (T3), FIT-161 (T13), FIT-195/196 (HADF 3a), FIT-154 (T6, shipped fitme-story #137), plus duplicates (FIT-128/FIT-64/FIT-65) and a permanently-blocked item miscatalogued as open (FIT-132, Code Connect on Pro plan). Notion's Roadmap simultaneously showed a stale feature count + a `foundation-models-tier3` "NOT merged" line that had merged via PR #724.

**The pattern:** Linear, Notion, `docs/product/backlog.md`, and the master/sub-plans are **downstream mirrors** of the repo, but nothing mechanically joined a tracker item to its repo artifact, so each mirror drifted independently. The repo (`.claude/features/<slug>/state.json::current_phase` + merged PRs) is the source of truth; a tracker status is a **claim**, not evidence. Compounded by the two-`R14`/`R9` scheme collisions (W-adjacent) which made even careful manual matching mis-fire (`r9-track-b-coverage-aggregator` ≠ dev-env `FIT-175`). Acting on a tracker's "open" without checking the repo risks **rebuilding shipped work** (the near-miss on R8) or scheduling on stale state (the F2 Phase-0 reality-check class).

**Detection / remediation:**

- **Verify-first, always:** before starting any tracker item, confirm against the repo — `current_phase` of the matching feature dir, plus `gh pr list --search` / git log for a merged PR or shipped script/workflow. An "open" item with a complete feature dir or merged PR is stale-open → close it, don't build it.
- **Mechanize the join:** the FW-NAMING convention (FIT-200, [`docs/process/cross-layer-item-naming-convention.md`](../../docs/process/cross-layer-item-naming-convention.md)) gives every item slug (canonical) + `state.json.linear_id` (FIT-NNN) + scheme-prefixed code. `make crosswalk` ([`scripts/build-item-registry.py`](../../scripts/build-item-registry.py)) builds `.claude/shared/item-registry.json` joining them + an advisory for features missing the join. For joined features the drift is now O(1) queryable instead of a manual sweep.
- **Never reuse a bare thematic number across schemes** (`R14`/`R9`/`F*`/`T*`) — always prefix (`DE-R14` vs `TC-R9`); the bare-number collisions are what made the backfill matcher mis-map.
- When reconciling, mark **shipped→Done, duplicate→Canceled, permanently-blocked→Won't-Do/Canceled** (not Done), and keep genuinely-overdue work open (e.g. the external-audit program FIT-201).
