...
| `W47` | Gate-accepted vs log-writer event vocabularies diverge for the same phase transition *(probed)* | yes | Use --event-type phase_started: it is the one value in BOTH the gate's PHASE_TRANSITION_EVENT_TYPES and append-feature-log.py's KNOWN_EVENT_TYPES. phase_transition warns at write; phase_completed fails the gate. |

At activation run `make skill-preflight SKILL=pm-workflow` — probes the 22 mechanized blockers for this work type; clear any before proceeding.
