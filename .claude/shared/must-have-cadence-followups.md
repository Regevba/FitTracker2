...
| B12 | **2026-05-27** | UCC hardening T+7d kill-criteria evaluation — populate `kill_criteria_resolution` in case study §4 + frontmatter; decide PROMOTE / RECALIBRATE / ROLLBACK. **T-2 prep done 2026-05-25 (PR #483):** case study §4 carries structured query templates + verdict decision matrix. **T-1 spec-bug hardening done 2026-05-26 (PR #499):** 3 query bugs caught by dry-run + fixed (`.event` → `.event_type`, ms-suffix timestamp trim, `.email_hash` → `.operator_label_hash`). Dry-run predicts **K1=not_fired + K2=not_fired + K3=not_yet_observed → predicted verdict PROMOTE**. Reconcile context: PR #497 flipped 24/26 task statuses to done (T20 + T26 = operator-only). ~30 min mechanical paste-work tomorrow. | operator | [`docs/case-studies/ucc-passkey-auth-security-hardening-case-study.md`](../../docs/case-studies/ucc-passkey-auth-security-hardening-case-study.md) §4 + §99 |
