...
| B15 | **2026-06-21** | T14 `PLATFORMS_TESTED` advisory→enforced calibration review — after the 14-day window (shipped advisory 2026-06-07 via PR #662), verify §2.2 criteria on the isolated `PLATFORMS_TESTED` coverage key (≥7d emission, 0 false positives, no silent skips, reversible single-flag). If met, flip `PLATFORMS_TESTED_ADVISORY_MODE = False` (~v7.10) + populate `kill_criteria_resolution`; else extend/adjust. Also remaining: T7 fitme-story dev-guide+glossary, T8 showcase MDX (cross-repo). | operator | t14-platform-parity-state-field PRD §6 T9 |
