...
| B18 | **ASAP (blocks B19)** | **Provision `WORKFLOW_PR_TOKEN` PAT secret** (Option B, bot-PR CI-trigger fix). One-time operator action: create a fine-grained PAT (FitTracker2 · Contents R/W + Pull requests R/W) and `gh secret set WORKFLOW_PR_TOKEN --repo Regevba/FitTracker2`. Until set, the `integrity-cycle` (72h) + `framework-status-weekly` (Mon) crons keep opening **deadlocking** PRs (checks stay `expected`). Full steps: [`docs/setup/bot-pr-ci-trigger-setup.md`](../../docs/setup/bot-pr-ci-trigger-setup.md). Verify with `make bot-pr-health`. | operator | bot-pr-ci-trigger-setup.md |
