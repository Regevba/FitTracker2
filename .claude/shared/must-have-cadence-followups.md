...
| B21 | **2026-08-05** | External Audit #3 (operator-driven, fresh-chat auditor) — run via `scripts/audit/build_bundle.py` profile `external` + `docs/audits/prompts/{01-extraction,02-auditor}.md` | operator | infra-plan §5 + §3.8 |
| B22 | **2026-08-12** | Data Freshness Audit #1 (quarterly, T+90d) — assert gate emission keys ↔ function names ↔ test names all canonical; uses F17 `gate-last-fired.json` index; profile `freshness` | operator | infra-plan §3.5.3 + §5 |
...
| ~~B16~~ | ~~2026-07-13~~ | ~~AN-1B.1 `CSV_TAXONOMY_DRIFT` advisory→enforced review~~ **EXECUTED 2026-07-13 — VERDICT: PROMOTE.** All 4 §8.2 criteria GREEN: (1) ✅ **8 emission days** (2026-06-29→07-11, 12-day span, 38 candidate rows in `gate-coverage.jsonl`); (2) ✅ baseline burndown 27→0 (2026-06-29) + re-verified live drift **0** (146 enum values all documented); (3) ✅ 0 false positives (`total_firings=0` since burndown); (4) ✅ reversible single-flag. Flipped `CSV_TAXONOMY_DRIFT_ADVISORY_MODE = True → False` at [`scripts/check-state-schema.py`](../../scripts/check-state-schema.py); 38 gate tests pass; behavioral check confirms drift=0 commits unaffected, drifted event now blocks. Distinct from W9/SCHEMA_DIFF holds (those never fired in field) — CSV_TAXONOMY_DRIFT demonstrably fired on 27 real drifts during burndown. Rollback: set back to `True` on `chore/csv-taxonomy-drift-rollback`. Surfaced by 2026-07-13 morning integrity sweep. | operator | analytics-master-plan §8.2 + an-1b1 calibration-artifacts.md |
| ~~B18~~ | ~~ASAP~~ | ~~Provision `WORKFLOW_PR_TOKEN` PAT secret~~ **EXECUTED 2026-07-13** — fine-grained PAT provisioned (`WORKFLOW_PR_TOKEN` secret set 15:43:44Z). **Proven end-to-end:** dispatched `integrity-cycle` opened PR #887 authored by the PAT (triggered all 3 required checks — no more `expected` deadlock), committed a **signed** snapshot (`verified=true` via `createCommitOnBranch`), and **auto-merged with zero admin**. Both Option-B blockers closed: PAT-PR-triggers-CI (#884) + signed-commit-satisfies-required_signatures (#886). `make bot-pr-health` clean. Soak verdict tracked at B19 (2026-08-10); PAT rotation at B20 (~2026-10-11). Setup: [`docs/setup/bot-pr-ci-trigger-setup.md`](../../docs/setup/bot-pr-ci-trigger-setup.md). | operator | bot-pr-ci-trigger-setup.md |
| B19 | **2026-08-10** | **Option B soak-window verdict** (started 2026-07-13, ~4 weeks). Over the window, every `integrity-cycle` + `framework-status-weekly` PR should trigger the 3 required checks + auto-merge (no deadlocks). Check continuously via `make bot-pr-health` (exit 1 ⇒ a bot PR deadlocked). **Satisfied** ⇒ Option B is the permanent fix. **Not satisfied** (PRs still deadlock after the PAT is provisioned) ⇒ Option B failed; escalate to **Option A** (ruleset migration + signed API-commits per `bot-pr-ci-trigger-setup.md` §"Why not Option A"). Depends on B18. **INTERIM READOUT 2026-07-14 (on track):** mechanism proven — #887 (first post-Option-B integrity-cycle PR) was PAT-opened, signed via `createCommitOnBranch`, and **auto-merged with zero admin**. `make bot-pr-health` clean (0 deadlocks). No cron has fired a *fresh* PR since ship yet (integrity-cycle every 72h → next ~07-16; framework-status Mondays → next 07-20); those are the natural data points to watch. Also 2026-07-14: the daily-checkpoint launchd cron was hardened (`CRON_CONTEXT=1`, #892) after a phantom +512 regression — orthogonal to the bot-PR soak but same cron-context class. Tracking is mechanized (daily checkpoint runs the sweep's CI-automation layer + surfaces B19 from 07-27 when ≤14d). | operator | bot-pr-ci-trigger-setup.md + check-bot-pr-health.py |
| B20 | **~2026-10-11** | **Rotate `WORKFLOW_PR_TOKEN`** (~90d from B18 provisioning — adjust to the actual PAT expiry). On expiry the crons silently fall back to `GITHUB_TOKEN` and the deadlock returns; `make bot-pr-health` will flag it. Re-issue the fine-grained PAT + `gh secret set`. | operator | bot-pr-ci-trigger-setup.md |
| ~~B17~~ | ~~2026-07-23~~ | ~~T12/FIT-160 `SCHEMA_DIFF` advisory→enforced review~~ **EXECUTED 2026-07-20 (3d early) — VERDICT: PROMOTE.** All 4 §2.2 criteria GREEN: (1) ✅ **8 emission days** (07-10→07-19, 64 rows in `gate-coverage.jsonl`) — criterion 1 did NOT hold at advisory; the gate got **1 genuine field firing 2026-07-16** (candidates=1/checked=1/0-drift, the positive signal W9 lacked); (2) ✅ 0 false positives — `total_failure_snapshots=0`, the real fire correctly found 0 drift, ship-time 0-drift verified; (3) ✅ no silent skips — all 63 skips `no_schema_or_sync_files_staged` (legit); (4) ✅ reversible single-flag. Flipped `SCHEMA_DIFF_ADVISORY_MODE = True → False` at [`scripts/check-state-schema.py:225`](../../scripts/check-state-schema.py); 11/11 gate tests pass (`test_schema_diff_gate.py` + `test_try_repo_schema_diff.py` — try-repo proves blocking). FRAMEWORK-FACTS annotated enforced. Rollback: set flag back to `True` on `chore/b17-rollback` (<2 min). | operator | test-coverage-master-plan §4 T12 + FRAMEWORK-FACTS.md |
