...
| B17 | **~2026-07-23** | T12/FIT-160 `SCHEMA_DIFF` advisory→enforced review (T+14d from 2026-07-09 advisory ship, #862). Flip `SCHEMA_DIFF_ADVISORY_MODE = True → False` at [`scripts/check-state-schema.py`](../../scripts/check-state-schema.py) when all 4 §2.2 criteria hold: (1) ≥7d `SCHEMA_DIFF` coverage in `gate-coverage.jsonl` — **pending; may HOLD** (the gate only emits coverage when a `backend/supabase/migrations/*.sql` OR `SupabaseSyncService.swift` change is staged, so with no schema/sync commits in the window it holds at advisory like W9 — a measurable no-op, not a failure); (2) 0 false positives — ✅ **verified 0 drift on the real repo at ship** (code references only extant columns); (3) no silent skips — skip reasons `no_schema_or_sync_files_staged` / `schema_or_code_unparseable` are legit; (4) reversible single-flag (<2 min). Corrected scope: [`docs/superpowers/specs/2026-07-09-t12-schema-diff-gate-corrected-scope.md`](../../docs/superpowers/specs/2026-07-09-t12-schema-diff-gate-corrected-scope.md). **Remaining at flip: criterion 1 (coverage window, may HOLD) + criterion 3 (review).** | operator | test-coverage-master-plan §4 T12 + FRAMEWORK-FACTS.md |
