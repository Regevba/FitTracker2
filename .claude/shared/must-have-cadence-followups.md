...
| ~~C8~~ | ~~SessionStart preflight — detect stale `.vercel/project.json` pointing to legacy `fit-tracker2`~~ **CLOSED FT2-side 2026-05-23 via PR #454**. Shipped `scripts/check-vercel-project-link.sh` + SessionStart hook. **C8b follow-up:** mirror script + hook into fitme-story repo (one-line companion PR). | ucc-passkey-auth case study §99 quirk 4 | low (operability) | chore (FT2 + fitme-story `.claude/settings.json`) | ~~2026-05-22~~ FT2 shipped 2026-05-23; C8b TBD |
