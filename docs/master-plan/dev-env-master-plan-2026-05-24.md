...
| R9 | Add coverage instrumentation (Slather + c8 + coverage.py) | **[~]** IN PROGRESS (2026-05-24) | iOS `.slather.yml` + ai-engine `pytest-cov` config shipped via FT2 PR (TBD). Web `c8` ships via companion fitme-story PR. Makefile `coverage-report` aggregator + CI workflow deferred to followup (infra-glob — needs isolated worktree). |
