...
7. **(NEW 2026-05-12; RESOLVED 2026-07-24) Phase A enforcement** — should Phase A artifacts (spec + fixture + dispatch test) be enforced via a new pre-commit gate (`GATE_SPEC_INCOMPLETE`) OR via PR-review checklist only? **DECISION: checklist + existing surfaces, NOT a new gate.** A mechanical `GATE_SPEC_INCOMPLETE` would need fuzzy "a new gate was added" detection and would duplicate three existing mechanisms — `gate-catalog.py --check` (untested-gate surfacing), T16 gate-tier annotation, and the quarterly Data Freshness Audit (first run 2026-08-12, which already asserts spec↔code sync). Low-frequency failure mode + v7.8.4 humility → checklist. Full rationale + re-eval triggers: [`docs/superpowers/specs/2026-07-24-gate-spec-incomplete-disposition.md`](../superpowers/specs/2026-07-24-gate-spec-incomplete-disposition.md).
