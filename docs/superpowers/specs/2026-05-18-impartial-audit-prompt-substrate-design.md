...
1. **Bundle visibility in main repo.** Should `runs/YYYY-MM-DD-<model>/bundle.md` be committed to git, or stay gitignored with only the manifest committed? Argument for committing: public reproducibility — anyone can replay the audit prompt against the bundle. Argument against: 500K-token bundles bloat the repo. Decide before the first audit on 2026-05-22.
