...
> **Date:** 2026-04-25 (v7.6 update)
> **Status:** v7.6 — **Mechanical Enforcement**: promotes 4 silent agent-attention checks to write-time pre-commit failures (`PHASE_TRANSITION_NO_LOG`, `PHASE_TRANSITION_NO_TIMING`, `BROKEN_PR_CITATION` write-time, `CASE_STUDY_MISSING_TIER_TAGS`), adds per-PR review bot + weekly framework-status cron + append-only adoption history, and explicitly enumerates 5 mechanically unclosable Class B gaps. Extends v7.5 (Data Integrity Framework, 8 cooperating defenses), v7.1 (72h integrity cycle), v7.0 (HADF). Triggered by the 2026-04-21 Google Gemini 2.5 Pro independent audit; v7.5 closed the 9 Tier 1/2/3 items as policy and tooling, v7.6 closes the residual Class B → Class A gap by making the policy mechanically enforced.
