...
- [ ] **v5.1 Adaptive Batch Execution** — Design approved 2026-04-14. Serial streaming backbone + opportunistic parallel subagents with preflight cost estimator. Batch dispatch for audit sweeps (`/design audit --batch`, `/ux validate --batch`, `/analytics validate --batch`) and multi-feature same-phase execution. Config in `skill-routing.json` (`batch_templates` + `batch_execution`). 30K token parallel budget cap, max 3 subagents. Implementation scope: skill-routing.json schema, pm-workflow/SKILL.md batch protocol, preflight estimator logic.
