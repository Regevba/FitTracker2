...
> Last updated: 2026-04-30 (added: Readiness-Aware Training Alert — Smart Reminders v2 layer)
...
- [ ] **Framework v7.8 Branch Isolation** (state.json: `framework-v7-8-branch-isolation`, captured 2026-04-30, current_phase=research) — Triggered by the HADF Phase 2 fingerprinting incident (2026-04-30): a long-running launchd-driven background job was anchored to the canonical repo path instead of an isolated `feature/` worktree; the wrapper's relative writes resolved against launchd's WorkingDirectory, canonical jsonl landed in the wrong tree, and the campaign required mid-flight isolation + restart + remerge. Deliverables: (1) v7.8+ write-time + cycle-time gate `BRANCH_ISOLATION_VIOLATION`; (2) extension to `superpowers:using-git-worktrees` skill to auto-detect when a feature with an active branch is being touched from main; (3) pre-state.json-mutation guard for scheduled jobs verifying `branch + cwd + WorkingDirectory` resolve to the expected `.worktrees/<name>` (or sibling `/Volumes/DevSSD/FitTracker2-*`) tree; (4) `make verify-isolation` readout listing every active feature with expected branch + worktree path + actual git/launchd state; (5) documentation of which jobs/skills are exempt (e.g. framework-level meta-work that legitimately runs on main). 5 open questions captured in state.json. Chains 4 predecessors (v7.5 / v7.6 / v7.7 / hadf-infrastructure). Trigger memory: `~/.claude/projects/-Volumes-DevSSD-FitTracker2/memory/project_hadf_phase2_in_progress.md`.
...
- [ ] **CI parallel-clone simulator hang — root cause investigation (added 2026-04-30)** — Hosted GitHub Actions Build-and-Test intermittently fails because one of the parallel iPhone 16 Pro simulator clones lands in an unhealthy state where `XCUIElementQuery.element` / `app.buttons.matching(NSPredicate(...))` calls hang for 74-236 seconds instead of returning their expected result. Different test fails each run (random victim selection across parallel clones). Two surgical quarantines now ship — `HomeReadinessUITests.testHomeTabRendersInAuthenticatedReviewMode()` (PR #160 → fixed in PR #165) and `OnboardingUITests.testOnboardingFirstStepRendersIfNotComplete()` (PR #165) — both using `XCTSkipIf(NSUserName() == "runner", ...)` since the original PR #160 attempt with `ProcessInfo.environment["GITHUB_ACTIONS"]` silently failed (env vars don't propagate to simulator XCTRunner process; documented permanently in test comments + memory `project_ci_ui_test_investigation_2026_04_29.md`). Both quarantines are workarounds, not fixes. Root-cause investigation steps when picking this up: (1) Reproduce on macos-15 self-hosted runner with `xcrun simctl list devices` showing the same iPhone 16 Pro (iOS 18) clone configuration the GH-hosted runner uses; (2) Capture `~/Library/Logs/CoreSimulator/<sim-uuid>/system.log` during a hang to see what's blocking accessibility snapshotting (likely candidate: a system dialog like HealthKit permission prompt or iCloud Sign-In intercepting the foreground); (3) Attempt to disable parallel testing in xcodebuild (`-parallel-testing-enabled NO`) on CI to confirm the hang is parallelism-induced; (4) If parallelism is the trigger, evaluate whether reducing `-maximum-parallel-testing-workers` to 1 is acceptable for the trade-off (longer CI wall time, fewer flake reverts). Acceptance criterion: 5 consecutive Build-and-Test runs green on a feature branch without quarantines, OR a documented decision to keep quarantines permanent with a reduction strategy. Timeline: not blocking; defer until next CI infra sweep.
- [ ] **Quarantine `AuthPolishV2UITests` against the parallel-clone sim hang (added 2026-05-05)** — Third surfacing of the env-flake described in the line above. PR #223 (a state.json-only change) hit a 297s Build-and-Test failure in [`FitTrackerUITests/AuthPolishV2UITests.swift`](../../FitTrackerUITests/AuthPolishV2UITests.swift)::`testBiometricUnlockView_rendersUnderReviewFixture()` — same hosted-runner sim hang signature, on a PR with zero Swift changes. Admin-merged via `--admin` flag (merge `b151e18`, 2026-05-05). The whack-a-mole pattern (per-method `XCTSkipIf` per failure) is reactive; this entry asks for a single proactive change: add `XCTSkipIf(NSUserName() == "runner", "Skipping under hosted-runner parallel-clone sim hang env-flake — see backlog 'CI parallel-clone simulator hang'")` to `AuthPolishV2UITests.setUp()` so all 5 tests in the file (`testForgotPasswordRequestView_isReachableFromEmailLoginIfWired`, `testForgotPasswordCooldownView_isReachableViaSuccessfulRequest`, `testSetNewPasswordView_isReachableViaDeepLinkReturn`, `testBiometricActivationSheet_rendersUnderReviewFixture`, `testBiometricUnlockView_rendersUnderReviewFixture`) skip on hosted runners until the root-cause investigation lands. Local Xcode runs are unaffected (`NSUserName()` only returns `"runner"` under GitHub Actions). Effort: ~5min change + 1 PR. Acceptance: a follow-up PR shows Build-and-Test passing without skip + no flake hit on `AuthPolishV2UITests` in 3 consecutive runs.
