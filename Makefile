...
integrity-check:
	...
	@python3 scripts/refresh-gate-last-fired.py --quiet || true
...
# v7.9.1 F17: refresh per-gate `last_fired_at` index from gate-coverage.jsonl.
# Read-only derived artifact at .claude/shared/gate-last-fired.json.
# Independent target so operators can refresh on demand without paying the
# integrity-check cost. Empirical wall-clock: <1s for ~2k rows.
...
# Dilution-immune adoption comparison across SEVERAL anchors. Reports RAW %,
# COHORT-normalized % (features present at both anchors — apples-to-apples), and
# absolute numerators, plus explicit dilution attribution. A COHORT regression is
# real; a RAW-only drop is denominator dilution from corpus growth.
# CI mode: exits 2 on a cohort regression — `make integrity-multi-anchor EXIT_ON_REGRESSION=1`.
integrity-multi-anchor:
	@python3 scripts/integrity-multi-anchor.py --json .claude/shared/integrity-multi-anchor.json $(if $(EXIT_ON_REGRESSION),--exit-on-regression,)

# Unified telemetry data-layer (read-only OLAP). Ingests EVERY source — state corpus,
# Mechanism A gate-coverage, daily/weekly crons, F17 index, anchors, snapshots — joins
# them, and emits cross-source reconciliation + dilution-normalized adoption + a
# forward-decision digest. Stdlib-first; optional DuckDB backend for `--sql`.
# CI mode: `make integrity-data-lake EXIT_ON_ANOMALY=1` exits 2 on a HIGH/CRITICAL finding.
integrity-data-lake:
...
# R10 CI mode: read-only integrity + regression check (no snapshot, no ledger
# write). Exits non-zero on findings/blocking/regression so the daily-checkpoint
# GitHub Actions job can alert without depending on the operator's Mac.
daily-checkpoint-ci:
	python3 scripts/daily-integrity-checkpoint.py --ci
