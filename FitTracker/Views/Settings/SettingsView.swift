...
private enum SettingsCategory: String, CaseIterable, Hashable, Identifiable {
    ...
    var tint: Color {
        switch self {
        case .accountSecurity: .appAccentPrimary
...
struct SettingsView: View {
    ...
    private func summaryBadges(for category: SettingsCategory) -> [SettingsSummaryBadge] {
        ...
        case .accountSecurity:
            ...
            return [
                SettingsSummaryBadge(title: provider, tint: .appAccentPrimary),
                SettingsSummaryBadge(title: biometric, tint: settings.requireBiometricUnlockOnReopen ? .status.success : .status.warning)
            ]
        case .healthDevices:
        ...
        case .trainingNutrition:
            return [
                ...
                SettingsSummaryBadge(title: "Zone 2 \(dataStore.userPreferences.zone2LowerHR)-\(dataStore.userPreferences.zone2UpperHR)", tint: .accent.cyan)
            ]
        case .dataSync:
            return [
                SettingsSummaryBadge(title: cloudSync.status.rawValue, tint: syncTint),
                SettingsSummaryBadge(title: "\(dataStore.dailyLogs.count) Logs", tint: .appAccentPrimary)
            ]
        }
    }
                    ...
                    Button {
                        ...
                        HStack(spacing: 12) {
                            ...
                            Image(systemName: isStatsMetricVisible(metric) ? "checkmark.circle.fill" : "circle")
                                ...
                                .foregroundStyle(isStatsMetricVisible(metric) ? metric.tint : Color.appTextTertiary)
                        }
                        .padding(.vertical, 4)
                    }
                    .buttonStyle(.plain)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 24)
                .fill(Color.appSurface.opacity(featured ? 0.96 : 0.92))
                .overlay(
                    RoundedRectangle(cornerRadius: 24)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(subtitle)
                ...
                .foregroundStyle(isSelected ? Color.white.opacity(0.86) : Color.appTextSecondary)
                .fixedSize(horizontal: false, vertical: true)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(.vertical, 10)
